...
variable "topic-name" {
  type        = string
  default     = "sentinel-audit-test-topic"
...
resource "google_pubsub_topic" "sentinel-audit-test-topic" {
  count   = "${var.topic-name != "sentinel-audit-test-topic" ? 0 : 1}"
  name    = var.topic-name
  project = data.google_project.project.project_id
}
...
resource "google_pubsub_subscription" "sentinel-subscription" {
  project = data.google_project.project.project_id
  name    = "sentinel-subscription-audit-testlogs"
  topic   = var.topic-name
  depends_on = [google_pubsub_topic.sentinel-audit-test-topic]
}

resource "google_logging_project_sink" "sentinel-sink" {
  project    = data.google_project.project.project_id
  count      = var.organization-id == "" ? 1 : 0
  name       = "audit-test-logs-sentinel-sink"
  destination = "pubsub.googleapis.com/projects/${data.google_project.project.project_id}/topics/${var.topic-name}"
  depends_on = [google_pubsub_topic.sentinel-audit-test-topic]

  filter = "logName = projects/msccp-test/logs/cloudaudit.googleapis.com%2Factivity OR logName = projects/msccp-test/logs/cloudaudit.googleapis.com%2Fdata_access OR logName = projects/msccp-test/logs/cloudaudit.googleapis.com%2Fsystem_event OR logName = projects/msccp-test/logs/cloudaudit.googleapis.com%2Fpolicy OR logName = projects/msccp-test/logs/cloudaudit.googleapis.com%2Faccess_transparency"
  unique_writer_identity = true
}

resource "google_logging_organization_sink" "sentinel-organization-sink" {
  count = var.organization-id == "" ? 0 : 1
  name   = "audit-test-logs-organization-sentinel-sink"
